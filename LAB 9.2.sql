...
-- Y5
select title, format(avg(rating), 4) as avg_rating
from series s
	join reviews r
    on s.id = r.series_id
group by title
...
-- Y6
select genre, format(avg(rating),2) as avg_rating
from series s
join reviews r
	on s.id = r.series_id
group by genre;
...
select first_name, last_name, count(rating) as COUNT, 
    ...
    format(ifnull(avg(rating), 0), 2) as AVG, 
